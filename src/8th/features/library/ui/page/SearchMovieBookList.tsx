...
const MOVIE_LEVELS = ['KA', 'KB', 'KC', '1A', '1B', '1C', '2C']
  ...
  const onStatusItemClick = (item: ActionBarDropdownItem) => {
    if (item.key.startsWith('Status')) {
      ...
      if (value.startsWith('Mode')) {
        ...
        setMovieSearch({ ...movieSearch, status: value, page: 1 })
      }
  ...
  return (
    <>
      <ActionBar
        ...
        count={searchCount}
        headerContent={
          <HeaderContent level={level} onLevelChange={onLevelChangeMovie} />
        }
        dropdowns={[
          {
            title: t('t8th012'),
...
function HeaderContent({
  ...
  const onLevelChanged = (option: string) => {
    const level = option
    if (onLevelChange) {
      onLevelChange(level)
    }
...
function isValidateParams(booktype: string, level: string): boolean {
  ...
  if (booktype === 'EB') {
    if (!MOVIE_LEVELS.includes(level)) {
      return false
    }
  }
  return true
