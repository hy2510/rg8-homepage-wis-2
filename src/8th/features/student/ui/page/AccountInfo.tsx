...
export default function AccountInfo() {
  // FIXME: Site 개별 설정 적용 필요 (country, isPaymentable, paymentUrl, isShowStudyEndDay, studentOpen)
    ...
    <>
      ...
      <BoxStyle
        ...
        <ExtraOptionLayoutStyle>
          {continuousViewType && (
            <ContinuousStudyView continuousViewType={continuousViewType} />
          )}
...
function StudentName({
  ...
  const onChangeStudentName = (name: string) => {
    ...
    if (!isValidateStudentNameKr(name) || name.length > 10) {
      alert(t('t8th106'))
      return
    }
