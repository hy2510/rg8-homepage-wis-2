...
async function getData() {
  const DATA_URL = `${STATIC_HOME_PAGE_RESOURCE}/contents/json/campaign.json`
  const CONTENTS_URL = SITE_PATH.HOME.EVENT_READING_CAMPAIN

  const postRes = await fetch(DATA_URL, { next: { revalidate: 3600 } })
